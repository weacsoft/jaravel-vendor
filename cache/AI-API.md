...
> Module: `cache` | Package: `com.weacsoft.jaravel.vendor.cache` | Version: 0.1.2
...
| `createDefaultStore`（static） | 无 | `CacheStore` | 静态工厂方法，创建默认的内存缓存 store（基于 `ArrayCacheDriver`，无前缀），供模块在 CacheManager 未注入时作为 fallback 使用 |
...
- **Description**: 基于内存的缓存驱动，对齐 Laravel "array" 缓存驱动。内部委托给 utils 模块的 `SimpleMemoryCache`（基于 ConcurrentHashMap + TTL 过期机制的零依赖内存缓存）实现，不再自带 ConcurrentHashMap + CacheEntry。线程安全，进程内有效，重启即失。读取时惰性清理过期条目。
- **Implements**: `CacheDriver`
- **Depends on**: `com.weacsoft.jaravel.vendor.utils.cache.SimpleMemoryCache`（utils 模块）
