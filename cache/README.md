...
> Jaravel-Vendor 的缓存模块，提供 Laravel 风格的 `Cache` 门面、`CacheStore` 仓库抽象、`ArrayCacheDriver`（内存）与 `FileCacheDriver`（文件）两种驱动，以及 `CacheManager` 多仓库管理。包名统一为 `com.weacsoft.jaravel.vendor.cache`。
...
- [11. CacheAutoConfiguration —— 自动装配](#11-cacheautoconfiguration--自动装配)
- [12. 配置选项](#12-配置选项)
- [13. 线程安全说明](#13-线程安全说明)
...
## 11. CacheAutoConfiguration —— 自动装配
...
| `cacheManager` | `CacheManager` | 缓存管理器，注册 `array` / `file` 两个 store 并设置默认 store |
