...
| `blacklistStore` | String | `""` | 黑名单使用的缓存 store 名称，为空时使用 cache 模块默认 store（仅 `blacklistEnabled=true` 时生效） |
...
黑名单使用的缓存 store 名称，默认为空字符串。为空时使用 cache 模块默认 store（由 `jaravel.cache.default-store` 决定）。仅当 `blacklistEnabled=true` 时生效。可选值：`array`、`file`、`redis`、`database`。
...
- `blacklistEnabled=true`：从 `CacheManager` 按 `JwtConfig.getBlacklistStore()` 指定的名称获取 store。若 `blacklistStore` 为空字符串，直接使用 cache 模块默认 store（`cacheManager.store()`）；若指定了名称但未注册，回退到默认 store。
...
jaravel:
  jwt:
    ...
    # 黑名单缓存 store，为空时使用 cache 模块默认 store
...
| `jaravel.jwt.blacklist-store` | String | `""` | 黑名单缓存 store 名称，为空时使用 cache 模块默认 store（仅黑名单开启时生效） |
