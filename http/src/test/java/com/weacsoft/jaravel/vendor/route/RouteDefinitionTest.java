...
/**
 * {@link RouteDefinition} URI 生成、fullUri 合并、命名空间与中间件测试。
 * 包含直接中间件和别名中间件两种模式。
 */
class RouteDefinitionTest {
    ...
    void testBasicRouteProperties() {
        ...
        RouteDefinition route = router.get("/users/{id}", NOOP);
    ...
    void testFullUriWithRouterPrefix() {
        ...
        RouteDefinition route = router.get("/users", NOOP);
    ...
    void testRouteNameAndFullName() {
        Router router = new Router();
        RouteDefinition route = router.get("/users", NOOP).name("users.index");
    ...
    void testFullNameWithGroupNamePrefix() {
        ...
        RouteDefinition route = router.getAllRoutes().get(0);
    ...
    void testRouteMiddleware() {
        ...
        Router router = new Router();
        RouteDefinition route = router.get("/secret", NOOP).middleware(m1, m2);
    ...
    void testFullUriNormalization() {
        Router router = new Router();
        RouteDefinition route = router.get("users//info", NOOP);
    ...
    void testRouteMiddlewareByAlias() {
        ...
        Router router = new Router();
        RouteDefinition route = router.get("/secret", NOOP).middleware("auth");
    ...
    void testRouteMiddlewareByAliasWithParameter() {
        ...
        Router router = new Router();
        RouteDefinition route = router.get("/api", NOOP).middleware("log:api");
    ...
    void testRouteMiddlewareMultipleAliasesInOrder() {
        ...
        Router router = new Router();
        RouteDefinition route = router.get("/api", NOOP).middleware("auth:api", "log", "cors");
    ...
    void testRouteMiddlewareMixedDirectAndAlias() {
        ...
        Router router = new Router();
        RouteDefinition route = router.get("/mixed", NOOP)
                .middleware(directMw)
                .middleware("auth")
                .middleware(directMw);
    ...
    void testRouteMiddlewareByClass() {
        ...
        Router router = new Router();
        RouteDefinition route = router.get("/log", NOOP).middleware(logMw.getClass());
    ...
    void testRouteMiddlewareByClassName() {
        ...
        RouteDefinition route = router.get("/log", NOOP).middleware(className);
