...
| starter | `starter` | 聚合Starter（引入即自动装配全部模块，jwt可选） | [README](starter/README.md) | [AI-API](starter/AI-API.md) |
...
| Maven Central 发布指南 | [MAVEN-CENTRAL-GUIDE.md](MAVEN-CENTRAL-GUIDE.md) | 从注册到发布的完整步骤 |
| settings.xml 模板 | [settings-template.xml](settings-template.xml) | Maven Central 认证 + GPG 签名配置模板 |
| API 文档站点 | [../jaravel-api-guide/index.html](../jaravel-api-guide/index.html) | 单个自包含 HTML 文件，25 个模块完整 API 参考 |
| Demo 项目 | `../jaravel/` | 展示全部 jaravel 能力的前后端分离示例项目（Laravel 文档风格） |

## 版本

当前版本：**0.1.0**（首个 Maven Central 发布版本）
