...
/**
 * JWT 自动装配：注册 JwtConfig、JwtService、JwtGuardDriver、JwtTokenResponseFilter Bean。
 * <p>
 * <b>工厂模式</b>：{@link JwtGuardDriver} 实现 {@code com.weacsoft.jaravel.vendor.auth.contract.AuthGuardDriver}，
 * 注册为 Spring Bean 后由 {@code vendor.springboot.auth.AuthAutoConfiguration} 自动收集并注册到 {@link AuthManager}，
 ...
 * 过期 token 在宽限期内仍可请求一次，{@code JwtTokenResponseFilter} 会自动将新 token
 * 写入响应 header。
 * <p>
 * Spring 装配收口于 springboot 模块（jwt 核心模块零 Spring 依赖）。
 */
@AutoConfiguration
...
public class JwtAutoConfiguration {
     ...
     * JWT 守卫驱动（工厂模式）。
     * <p>
     * 实现 {@code com.weacsoft.jaravel.vendor.auth.contract.AuthGuardDriver}，支持 "jwt" 驱动。
     * 由 {@code vendor.springboot.auth.AuthAutoConfiguration} 自动收集并注册到 {@link AuthManager}。
     */
    @Bean
    @ConditionalOnMissingBean
