...
import com.weacsoft.jaravel.vendor.event.EventDispatcher;
import com.weacsoft.jaravel.vendor.event.QueueDispatcher;
import com.weacsoft.jaravel.vendor.event.QueueManager;
import com.weacsoft.jaravel.vendor.event.QueuePublishableConfig;
 ...
 * <p>
 * Spring 装配收口于 springboot 模块（event 核心模块零 Spring 依赖）。
 *
 * <p>
 * 注册以下 Bean：
 * <ul>
 ...
 *   <li>{@link EventListenerRegistrar}：扫描 {@code @ListensTo} 注解自动注册监听器。</li>
 * </ul>
 * 并依据 {@link EventProperties} 配置异步分发开关、队列大小与重试参数
 * （经 {@link EventProperties#toEventConfig()} 映射为 event 模块的纯 Java {@code EventConfig}）。
 * <p>
...
public class EventAutoConfiguration {
    ...
    public QueueManager queueManager(EventProperties properties) {
        return new QueueManager(properties.toEventConfig());
    }
    ...
    /**
     * 注册监听器自动注册器，在所有单例就绪后扫描 {@code @ListensTo} 注解。
     *
