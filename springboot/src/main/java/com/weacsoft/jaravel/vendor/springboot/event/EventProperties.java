...
import com.weacsoft.jaravel.vendor.event.EventConfig;
 ...
 * </pre>
 * <p>
 * Spring 侧绑定对象；经 {@link #toEventConfig()} 映射为 event 模块的纯 Java {@link EventConfig}
 * 后交给 {@code QueueManager}（event 模块零 Spring 依赖）。
 */
...
public class EventProperties {
    ...
    /**
     * 映射为 event 模块的纯 Java {@link EventConfig}（字段一一对应，默认值一致）。
     *
     * @return 纯 Java 配置对象
     */
    public EventConfig toEventConfig() {
        EventConfig config = new EventConfig();
        config.setQueueEnabled(queueEnabled);
        if (queue != null) {
            EventConfig.Queue outQueue = new EventConfig.Queue();
            outQueue.setDefaultPoolSize(queue.getDefaultPoolSize());
            outQueue.setQueuePoolSizes(queue.getQueuePoolSizes());
            config.setQueue(outQueue);
        }
        if (retry != null) {
            EventConfig.Retry outRetry = new EventConfig.Retry();
            outRetry.setMaxAttempts(retry.getMaxAttempts());
            outRetry.setDelayMs(retry.getDelayMs());
            config.setRetry(outRetry);
        }
        return config;
    }
     ...
     * {@code queuePoolSizes} 为各命名队列的线程池大小覆盖，key 为队列名。
     */
