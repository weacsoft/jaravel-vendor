package com.weacsoft.jaravel.vendor.springboot.modelcache;
...
import com.weacsoft.jaravel.vendor.modelcache.ModelCachePublishableConfig;
import com.weacsoft.jaravel.vendor.modelcache.ModelCacheService;
 ...
 * 不影响其他模块。
 * <p>
 * {@link ModelCacheProperties} 为纯 POJO（留在 model-cache 模块，{@code ModelCacheService} 直接消费），
 * 通过 {@code @Bean @ConfigurationProperties} 方法完成绑定。
 * <p>
 * Spring 装配收口于 springboot 模块（model-cache 核心模块零 Spring 依赖）。
 */
@AutoConfiguration
...
public class ModelCacheAutoConfiguration {
    ...
    /**
     * 模型缓存配置 Bean，绑定 {@code jaravel.model-cache.*} 配置。
     *
     * @return 模型缓存配置
     */
    @Bean
    ...
    public ModelCacheProperties jaravelModelCacheProperties() {
        return new ModelCacheProperties();
    }
