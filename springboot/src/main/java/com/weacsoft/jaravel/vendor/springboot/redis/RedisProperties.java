...
import com.weacsoft.jaravel.vendor.redis.RedisConfig;
 ...
 * <p>
 * Spring 侧绑定对象；经 {@link #toRedisConfig()} 映射为 redis 模块的纯 Java
 * {@link RedisConfig} 后交给 {@code RedisManager}（redis 模块零 Spring 依赖）。
 *
 * <pre>
 * jaravel:
 *   redis:
...
public class RedisProperties {
    ...
    /**
     * 映射为 redis 模块的纯 Java {@link RedisConfig}（字段一一对应，默认值一致）。
     *
     * @return 纯 Java 配置对象
     */
    public RedisConfig toRedisConfig() {
        ...
        RedisConfig.Options outOptions = new RedisConfig.Options();
        if (options != null) {
            outOptions.setCluster(options.getCluster());
            outOptions.setPrefix(options.getPrefix());
        }
        config.setOptions(outOptions);
        Map<String, RedisConfig.ConnectionConfig> outConnections = new LinkedHashMap<>();
        if (connections != null) {
            connections.forEach((name, in) -> {
                RedisConfig.ConnectionConfig out = new RedisConfig.ConnectionConfig();
                if (in != null) {
                    ...
                    out.setDatabase(in.getDatabase());
                    out.setTimeoutMs(in.getTimeoutMs());
                    out.setSentinelMaster(in.getSentinelMaster());
                    out.setSentinels(in.getSentinels());
                    out.setClusterNodes(in.getClusterNodes());
                }
                outConnections.put(name, out);
            });
        }
        config.setConnections(outConnections);
        return config;
    }
