...
/**
 * 事件模块纯 Java 配置对象（零 Spring 依赖），字段与默认值与
 * Spring 侧的 {@code vendor.springboot.event.EventProperties}（前缀 {@code jaravel.event}）一一对应，
 * 经 {@code EventProperties.toEventConfig()} 映射后交给 {@link QueueManager}。
 * <p>
 * 对齐 Laravel {@code config/event.php} 与 {@code config/queue.php}。
