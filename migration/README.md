...
模块支持四种迁移来源（`MigrationSource` 枚举），适配不同的部署环境：
...
> 运行环境要求：JDK 17+（DIRECTORY 模式）或 JRE 17+（DIRECTORY_CLASSES / JAR / CLASSPATH 模式），Spring Boot 3.2.5（Spring 6.x），容器中需存在 `javax.sql.DataSource` Bean。
...
├── MigrationSource              // 迁移源类型枚举（DIRECTORY / DIRECTORY_CLASSES / JAR / CLASSPATH）
├── MigrationScanner             // 迁移源扫描器与加载器（支持四种模式）
...
四种模式均通过 `@MigrationAnnotation` 注解自动识别迁移类，无需手动指定包名。

---

## 4. 迁移源模式

`migration` 模块支持四种迁移来源模式，通过 `jaravel.migration.source` 配置项切换，适配不同的部署环境（开发 / 生产 / 内置迁移）。
...
- 适用场景：生产部署，迁移类已在开发阶段预编译为 `.class` 文件（通过 `javac` 或构建工具编译）
- 配置项：`jaravel.migration.directory`（指向包含 `.class` 文件的目录）
...
四种迁移源模式均支持**自动包名检测**：迁移文件可以使用任意包名，`MigrationScanner` 会自动识别并加载。无需手动配置包扫描路径。

- DIRECTORY 模式：从 `.java` 源文件中解析 `package` 声明提取包名
- DIRECTORY_CLASSES / JAR / CLASSPATH 模式：扫描所有 `.class` 文件，通过 `@MigrationAnnotation` 注解判断是否为迁移类，自动识别任意包名
...
| --- | --- | --- | --- |
| `jdk-mode` | 默认（未指定 `-Djre-only` 时激活） | 完整 JDK | DIRECTORY / DIRECTORY_CLASSES / JAR / CLASSPATH |
| `jre-mode` | `-Djre-only` | 仅 JRE | DIRECTORY_CLASSES / JAR / CLASSPATH |

### jdk-mode（默认）

完整 JDK 环境，`javax.tools.JavaCompiler` 由 JDK 提供，支持全部四种迁移源模式（包括 DIRECTORY 内存编译）。
...
仅 JRE 环境，不包含编译器，因此不支持 DIRECTORY 内存编译模式，仅支持 DIRECTORY_CLASSES、JAR 与 CLASSPATH 模式。
...
> 2. 切换为 `JAR` 模式，使用预编译 jar 包
> 3. 切换为 `CLASSPATH` 模式，将迁移类打包进主 jar
> 4. 安装完整 JDK 环境
...
jaravel:
  migration:
    enabled: true
    source: DIRECTORY          # DIRECTORY / DIRECTORY_CLASSES / JAR / CLASSPATH
    directory: migrations
    table: migrations
    auto-run: true
```
...
> **重要**：迁移类使用 `@MigrationAnnotation` 标记（非 Spring `@Component`），在运行时由 `MigrationScanner` 加载（DIRECTORY 内存编译 / DIRECTORY_CLASSES 加载 / JAR 加载 / CLASSPATH 扫描）、反射实例化、执行后自动释放。
...
迁移类标记注解，用于在运行时识别哪些类是迁移类。**替代 Spring `@Component`**：迁移文件不再作为 Spring Bean 注册到容器，而是通过 `MigrationScanner` 在运行时加载（DIRECTORY 编译 / DIRECTORY_CLASSES 加载 / JAR 加载 / CLASSPATH 扫描）、反射实例化、执行后自动释放。
...
> 四种迁移源模式均通过 `@MigrationAnnotation` 自动识别迁移类，迁移文件可使用任意包名，无需手动配置包扫描路径。
...
迁移源扫描器与加载器，支持四种迁移来源模式（DIRECTORY / DIRECTORY_CLASSES / JAR / CLASSPATH）。四种模式均通过 `@MigrationAnnotation` 注解自动识别迁移类，无需手动指定包名。
...
| `loadedClasses` | `Map<String, Class<?>>` | DIRECTORY_CLASSES / JAR / CLASSPATH 模式加载的类（类名 -> Class） |
...
| `void loadFromDirectoryClasses(String dirPath)` | DIRECTORY_CLASSES | **新增**：从目录加载预编译 `.class` 文件（仅需 JRE），递归扫描所有 `.class` |
| `void loadFromDirectoryClasses(File dir)` | DIRECTORY_CLASSES | **新增**：同上，接收 `File` 参数 |
...
| `List<String> getAllMigrationClassNames()` | 通用 | 获取所有迁移类全限定名列表（含 DIRECTORY 编译的与 DIRECTORY_CLASSES/JAR/CLASSPATH 加载的） |
...
对齐 Laravel `Illuminate\Database\Migrations\Migrator`。通过 `MigrationScanner` 在运行时加载迁移类（DIRECTORY 编译 / DIRECTORY_CLASSES 加载 / JAR 加载 / CLASSPATH 扫描）、反射实例化，按名称排序，与 `MigrationRepository` 记录比对，执行待运行的迁移或回滚。所有操作完成后调用 `finish()` 释放资源。
...
> **重要变更**：不再通过 Spring DI 注入 `List<Migration>`，而是在运行时创建 `MigrationScanner`，根据 `jaravel.migration.source` 配置选择加载方式（DIRECTORY 编译 / DIRECTORY_CLASSES 加载 / JAR 加载 / CLASSPATH 扫描），通过 `Migrator` 执行迁移，完成后调用 `scanner.finish()` 释放资源。
...
> **重要变更**：不再注入 `List<Migration>`，也不再注册 `Schema`、`MigrationRepository`、`Migrator` 为 Bean。迁移文件不再是 Spring 组件，而是通过 `MigrationScanner` 在运行时加载（DIRECTORY 编译 / DIRECTORY_CLASSES 加载 / JAR 加载 / CLASSPATH 扫描）、反射实例化、执行后自动释放。本类仅注册 `MigrationRunner` 一个 Bean，注入 `DataSource` 与 `MigrationProperties`。
...
jaravel:
  migration:
    source: DIRECTORY        # 迁移源类型（DIRECTORY / DIRECTORY_CLASSES / JAR / CLASSPATH）
    directory: migrations   # 迁移文件目录（DIRECTORY 为 .java，DIRECTORY_CLASSES 为 .class）
...
| 属性 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `jaravel.migration.source` | `MigrationSource` | `DIRECTORY` | 迁移源类型，可选 `DIRECTORY` / `DIRECTORY_CLASSES` / `JAR` / `CLASSPATH` |
| `jaravel.migration.directory` | `String` | `migrations` | 迁移文件目录。DIRECTORY 模式为 `.java` 文件目录，由 `MigrationScanner` 扫描并内存编译；DIRECTORY_CLASSES 模式为 `.class` 文件目录，由 `MigrationScanner` 加载预编译类 |
