...
/**
 * Redis 纯 Java 配置对象（零 Spring 依赖），字段与默认值与
 * Spring 侧的 {@code vendor.springboot.redis.RedisProperties}（前缀 {@code jaravel.redis}）一一对应，
 * 经 {@code RedisProperties.toRedisConfig()} 映射后交给 {@link RedisManager}。
 * <p>
 * 对齐 Laravel {@code config/database.php} 的 redis 段。
