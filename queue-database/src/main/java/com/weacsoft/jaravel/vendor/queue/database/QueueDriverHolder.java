...
public class QueueDriverHolder implements QueueDriver {
    ...
    public long push(String queueName, String payload) {
        return require().push(queueName, payload);
    }

    @Override
    public long push(String queueName, String payload, long delayMs) {
        return require().push(queueName, payload, delayMs);
    }

    @Override
    public QueuedJob pop(String queueName) {
        return delegate == null ? null : delegate.pop(queueName);
    }

    @Override
    public void delete(long jobId) {
        require().delete(jobId);
    }

    @Override
    public void release(long jobId) {
        require().release(jobId);
    }

    @Override
    public void release(long jobId, long delayMs) {
        require().release(jobId, delayMs);
    }

    @Override
    public int size(String queueName) {
        return delegate == null ? 0 : delegate.size(queueName);
    }
    ...
    public void fail(long jobId, String queue, String payload, int attempts, String exception) {
        require().fail(jobId, queue, payload, attempts, exception);
    }

    @Override
    public List<QueuedJob> getFailedJobs() {
        return delegate == null ? Collections.emptyList() : delegate.getFailedJobs();
    }

    @Override
    public void retryFailedJob(long failedJobId) {
        require().retryFailedJob(failedJobId);
    }

    @Override
    public void deleteFailedJob(long failedJobId) {
        require().deleteFailedJob(failedJobId);
    }

    @Override
    public void clearFailedJobs() {
        if (delegate != null) {
            delegate.clearFailedJobs();
        }
    }
}
